...
```
[GCP VM - e2-medium]
  ├── Java 17 + Spring Boot  →  ~/demo-vector-springboot/logs/app.log (JSON)
...
|-----------|----------------------|
| `build.gradle` | `~/demo-vector-springboot/build.gradle` |
| `settings.gradle` | `~/demo-vector-springboot/settings.gradle` |
| `src/main/resources/application.yml` | `~/demo-vector-springboot/src/main/resources/application.yml` |
| `src/main/resources/logback-spring.xml` | `~/demo-vector-springboot/src/main/resources/logback-spring.xml` |
| `src/main/java/.../VectorTestApplication.java` | `~/demo-vector-springboot/src/main/java/com/example/vectortest/VectorTestApplication.java` |
| `src/main/java/.../LogTestController.java` | `~/demo-vector-springboot/src/main/java/com/example/vectortest/controller/LogTestController.java` |
| `vector/vector.yaml` | `~/demo-vector-springboot/vector/vector.yaml` |

업로드 후 디렉토리 구조 생성 및 파일 이동:
```bash
# 디렉토리 구조 생성
mkdir -p ~/demo-vector-springboot/src/main/java/com/example/vectortest/controller
mkdir -p ~/demo-vector-springboot/src/main/resources
mkdir -p ~/demo-vector-springboot/vector
mkdir -p ~/demo-vector-springboot/logs

# 업로드된 파일은 기본적으로 홈 디렉토리(~/)에 위치함
# 각 파일을 올바른 경로로 이동
mv ~/build.gradle ~/demo-vector-springboot/
mv ~/settings.gradle ~/demo-vector-springboot/
mv ~/application.yml ~/demo-vector-springboot/src/main/resources/
mv ~/logback-spring.xml ~/demo-vector-springboot/src/main/resources/
mv ~/VectorTestApplication.java ~/demo-vector-springboot/src/main/java/com/example/vectortest/
mv ~/LogTestController.java ~/demo-vector-springboot/src/main/java/com/example/vectortest/controller/
mv ~/vector.yaml ~/demo-vector-springboot/vector/
```
...
git clone https://github.com/YOUR_REPO/demo-vector-springboot.git ~/demo-vector-springboot
```
...
# 예시: vector.yaml 생성
cat > ~/demo-vector-springboot/vector/vector.yaml << 'EOF'
# 로컬 PC의 vector/vector.yaml 내용을 여기에 붙여넣기
EOF

# 예시: build.gradle 생성
cat > ~/demo-vector-springboot/build.gradle << 'EOF'
...
```bash
nano ~/demo-vector-springboot/vector/vector.yaml
```
...
### 7-3. data_dir 생성 및 설정 파일 문법 검증

```bash
# data_dir 먼저 생성 (없으면 validate 실패)
mkdir -p /tmp/vector-data

vector validate --config-yaml ~/demo-vector-springboot/vector/vector.yaml
# √ Validated  출력 확인
```
...
```bash
nohup java -jar build/libs/vector-test-app-0.0.1-SNAPSHOT.jar > /tmp/springboot.out 2>&1 &
...
```bash
nohup vector --config ~/demo-vector-springboot/vector/vector.yaml \
  > /tmp/vector.out 2>&1 &
...
INFO vector::sources::file: Watching file  path="/home/user/demo-vector-springboot/logs/app.log"
...
```bash
tail -f ~/demo-vector-springboot/logs/app.log
```
...
### ❌ `fingerprint` 설정 오류 (ignored_header_bytes missing)

**증상:** `validate` 실행 시 `missing field 'ignored_header_bytes' in sources.app_logs`

**해결:** `vector.yaml` 에서 `fingerprint:` 블록 전체 제거
```yaml
# 삭제 대상
    fingerprint:
      strategy: checksum
...
### ❌ `data_dir` 경로가 존재하지 않는 경우

**증상:** `validate` 실행 시 `data_dir "/tmp/vector-data" does not exist`
...
```bash
# 파일 경로 및 권한 확인
ls -la ~/demo-vector-springboot/logs/app.log

# vector.yaml 의 include 경로를 절대 경로로 수정
# /home/user/demo-vector-springboot/logs/app.log  (상대 경로 사용 금지)
```
...
```bash
# 1. Vector 상세 로그 확인
VECTOR_LOG=debug vector --config ~/demo-vector-springboot/vector/vector.yaml 2>&1 | grep -E "error|abort|parse"
...
```bash
# inotify 동작 확인
inotifywait -m ~/demo-vector-springboot/logs/app.log
...
sources:
  app_logs:
    type: file
    include:
      - /home/user/demo-vector-springboot/logs/app.log
